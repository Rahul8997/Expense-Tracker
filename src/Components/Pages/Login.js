...
const Login = () => {
    ...
    const submitHandler = async (e) => {
        ...
        if (!login) {
            if (enteredEmail.current.value.length > 0 && enteredpass.current.value.length > 0 && enteredConfirmPass.current.value.length > 0) {
                if (enteredpass.current.value === enteredConfirmPass.current.value) {
                    let responce = await fetch(
                        `https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${process.env.REACT_APP_FIREBASE_PROJECT_API}`,
                        {
                            method: 'POST',
                            body: JSON.stringify({
            ...
            if (enteredEmail.current.value.length > 0 && enteredpass.current.value.length > 0) {
                let responce = await fetch(
                    `https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${process.env.REACT_APP_FIREBASE_PROJECT_API}`,
                    {
                        method: 'POST',
                        body: JSON.stringify({
                            email: enteredEmail.current.value,
                ...
                if (responce.ok) {
                    ...
                    try {
                        let responce = await fetch(
                            `https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=${process.env.REACT_APP_FIREBASE_PROJECT_API}`,
                            {
                                method: 'POST',
                                body: JSON.stringify({
    ...
    return (
        <div className='row'>
            <div className="mx-auto my-5 bg-light col-sm-9 col-md-5 col-10 col-lg-3">
                <h2 className='my-3 text-center text-dark'>{!login ? "Sign Up" : "Log In"}</h2>
                <form >
                    ...
                    <div >
                        ...
                        {login && <Link className='nav-link active text-center' to="/forget">forget password?</Link>}
                    </div>
                </form>
                <div>
                    <button
